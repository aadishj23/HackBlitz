...
function Navbar(){
    ...
    return(
        ...
        <div className="flex py-3 border-b-2 border-opacity-50 border-white max-lg:justify-between ">
            ...
            <button onClick={()=>setNavBarOpen(!navBarOpen)} className="mr-4 lg:hidden">
            ...
            <ul className={`bg-black text-white border-t-2 lg:hidden ${navBarOpen ? "block" : "hidden"}`}>
                    {items.map((item,index)=>(
                        <li key={index} className="py-4 text-center border-b-2 border-opacity-50 border-gray-400">{item}</li>
                    ))}
            </ul>
        </>
    )
